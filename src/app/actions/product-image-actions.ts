...
async function requireAdminOrSeller() {
  ...
  if (!profileRow || !["admin", "seller"].includes(profileRow.role || "")) {
    return {
      supabase,
      user: null,
      role: null,
  ...
  return { supabase, user, role: profileRow.role, error: null };
}
...
async function verifyProductOwnershipOrAdmin(
  ...
  const productRow = product as { id?: string; seller_id?: string } | null;

  return !!productRow && productRow.seller_id === userId;
}
...
export async function addProductImageAction(formData: FormData) {
  ...
  const lastImageRow = lastImage as { sort_order?: number } | null;
  const nextSortOrder = (lastImageRow?.sort_order ?? 0) + 1;
