...
import { canManageProduct } from "@/lib/auth/ownership";
...
export async function updateSellerProductAction(formData: FormData) {
  ...
  const { supabase, allowed, role } = await canManageProduct(productId);

  if (!allowed || role !== "seller") {
    return { error: "You can only edit your own seller products." };
  }
